...
/* ---------- helpers ---------- */
const uniqueGenres = list => ['Todos', ...new Set(list.flatMap(g => g.genero || []))];
...
function sortList(list) {
  switch (state.sort) {
    case 'Popularidade':            return favorites.rankByPopularity(list);
...
async function bootstrap() {
  ...
  // popula selects (apenas 1x e mantém "Popularidade")
  ...
  $('#price').innerHTML = ['Todos','Até R$50','R$50–R$80','R$80–R$120','R$120+']
  ...
  const onChange = () => { render(); writeParams(); };

  $('#search').addEventListener('input', debounce(e => {
    state.search = e.target.value; onChange();
  }, 250));
  $('#genre').addEventListener('change', e => { state.genre = e.target.value; onChange(); });
  $('#price').addEventListener('change', e => { state.price = e.target.value; onChange(); });
  $('#sort').addEventListener('change',  e => { state.sort  = e.target.value; onChange(); });
  $('#onlyFav').addEventListener('change', e => { state.fav = e.target.checked; onChange(); });
